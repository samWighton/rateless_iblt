...
T: symbol::Symbol,
{
    ...
    for item in iterable.into_iter() {
        ...
        for i in item_mapping.take_while(|&x| x < offset+BLOCK_SIZE).filter(|&x| x >= offset) {
            block[(i - offset) as usize].apply(&symbol::HashedSymbol::new(item.clone()), symbol::Direction::Add);
...
pub fn remove_symbol_from_block<T: symbol::Symbol>(
    ...
    symbol_result: symbol::PeelableResult<T>,
        ...
        for i in item_mapping.take_while(|&x| (x as usize) < block_len) {
            block[i as usize].apply(&symbol::HashedSymbol::new(symbol.clone()), direction.clone());
        }
}
...
pub fn collapse<T: symbol::Symbol>(
    ...
    for (coded_symbol_local, coded_symbol_remote) in block_local.iter().zip(block_remote.iter()) {
        combined_block.push(
            coded_symbol_local.collapse(coded_symbol_remote)
        );
    }
    combined_block
}
